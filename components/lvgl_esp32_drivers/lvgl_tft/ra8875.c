...
#define VDHR_VAL ( V_OFFSET + (CONFIG_LVGL_DISPLAY_HEIGHT - 1) )
...
void ra8875_init(void)
{
    ...
    struct {
        ...
        {RA8875_REG_VDHR0,  ((VDHR_VAL >> 0) & 0x00FF)},         // LCD Vertical Display Height Register (VDHR0)
        {RA8875_REG_VDHR1,  ((VDHR_VAL >> 8) & 0x00FF)},            // LCD Vertical Display Height Register0 (VDHR1)
        {RA8875_REG_VNDR0,  ((VNDR_VAL >> 0) & 0x00FF)},         // LCD Vertical Non-Display Period Register (VNDR0)
        {RA8875_REG_VNDR1,  ((VNDR_VAL >> 8) & 0x00FF)},            // LCD Vertical Non-Display Period Register (VNDR1)
        {RA8875_REG_VSTR0,  ((VSTR_VAL >> 0) & 0x00FF)},         // VSYNC Start Position Register (VSTR0)
        {RA8875_REG_VSTR1,  ((VSTR_VAL >> 8) & 0x00FF)},            // VSYNC Start Position Register (VSTR1)
...
static void ra8875_set_window(unsigned int xs, unsigned int xe, unsigned int ys, unsigned int ye)
{
    ...
    ra8875_write_cmd(RA8875_REG_VSAW0, (uint8_t)((ys + V_OFFSET) & 0x0FF)); // Vertical Start Point 0 of Active Window (VSAW0)
    ra8875_write_cmd(RA8875_REG_VSAW1, (uint8_t)((ys + V_OFFSET) >> 8));    // Vertical Start Point 1 of Active Window (VSAW1)
    ...
    ra8875_write_cmd(RA8875_REG_VEAW0, (uint8_t)((ye + V_OFFSET) & 0x0FF)); // Vertical End Point of Active Window 0 (VEAW0)
    ra8875_write_cmd(RA8875_REG_VEAW1, (uint8_t)((ye + V_OFFSET) >> 8));    // Vertical End Point of Active Window 1 (VEAW1)
}
...
static void ra8875_set_memory_write_cursor(unsigned int x, unsigned int y)
{
    ...
    ra8875_write_cmd(RA8875_REG_CURV0, (uint8_t)((y + V_OFFSET) & 0x0FF));  // Memory Write Cursor Vertical Position Register 0 (CURV0)
    ra8875_write_cmd(RA8875_REG_CURV1, (uint8_t)((y + V_OFFSET) >> 8));     // Memory Write Cursor Vertical Position Register 1 (CURV1)
}
